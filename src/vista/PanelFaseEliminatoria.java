...
public class PanelFaseEliminatoria extends JPanel {
	...
	public PanelFaseEliminatoria(ArrayList<Entrenador> clasificados) {
		...
		if (clasificados.size()==1) {
			...
			ganador.setBounds(0, 0, 200, 200);
			ganador.setPreferredSize(new Dimension(100,100));
			ganador.setBorder(new BevelBorder(BevelBorder.RAISED));
			JLabel nombreganador=new JLabel(clasificados.get(0).getNombre());
			ganador.add(nombreganador);
			this.panelCentral.add(ganador);
		}
